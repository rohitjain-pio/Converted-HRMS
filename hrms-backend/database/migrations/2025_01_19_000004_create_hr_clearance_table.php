...
return new class extends Migration
{
    ...
    {
        Schema::create('hr_clearance', function (Blueprint $table) {
            $table->bigInteger('Id')->autoIncrement()->primary();
            $table->integer('ResignationId');
            $table->decimal('AdvanceBonusRecoveryAmount', 18, 2);
            $table->text('ServiceAgreementDetails')->nullable();
            $table->decimal('CurrentEL', 5, 2)->nullable();
            $table->integer('NumberOfBuyOutDays');
            $table->boolean('ExitInterviewStatus')->nullable();
            $table->text('ExitInterviewDetails')->nullable();
    ...
    {
        Schema::dropIfExists('hr_clearance');
    }
};
