...
} from "@/pages/AssetManagement/components/ImportConfirmationDialog";
import { ImportEmployeesDataResponse } from "@/services/AssetManagement/types";
import { importEmployeesAsset } from "@/services/AssetManagement/assetManagementService";
...
export const ImportButton = (props: ImportButtonProps) => {
  ...
  >({
    requestFn: async (args: {
      ...
      return await importEmployeesAsset(file, isImportConfirmed);
    },
    onSuccess: (response, params) => {
      ...
      const [firstLine, secondLine] = message.split(",", 2);

      toast.success(
        <>
          {`${firstLine.trim()} and ${secondLine?.trim() || ""}`}         
        </>
      );
